...
### 2.2 The band work exposed a calibration artifact we are correcting
The clipping diagnostic proved the sigmoid outputs **saturate** — residuals are mechanically capped by `1 − stored`. That is not an artifact to work around with one-sided bands; it is a symptom that **the model's probabilities are not calibrated in the 0.6–0.9 stored-value zone**. We built an elegant band structure on top of an uncalibrated probability. Review has now confirmed the full extent: the **two-sided band was a sigmoid-clipping artifact**, and the one-sided aggregate that replaced it was **decile-carried** — aggregate onset coverage 0.822 was held up by clipped-to-zero rows, with only 1–28% coverage across the 0.5–0.9 deciles. Those numbers are withdrawn, not publishable. The fix is calibration at the root — split-conformal **per-decile** bands on calibrated probabilities — not another downstream band patch.
...
- **Band calibration artifact found and being corrected (fixes 2.2):** see §2.2 — the two-sided band (sigmoid-clipping artifact) and the one-sided aggregate (decile-carried; aggregate coverage 0.822 held up by clipped-to-zero rows, 0.5–0.9 deciles only 1–28%) are **withdrawn from all text**. **Correction in progress:** split-conformal per-decile bands on calibrated probabilities. **Bands are not yet publishable; confidence thresholds and language are frozen** until the rebuild verifies. Lexicon remains "historical range, not a guarantee".
