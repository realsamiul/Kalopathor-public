...
- **Calibration & band rebuild** — review finding: the two-sided band was a sigmoid-clipping artifact and the one-sided aggregate was **decile-carried** (aggregate onset coverage 0.822 held up by clipped-to-zero rows; 0.5–0.9 deciles only 1–28% coverage). Band numbers withdrawn. **Rebuild in progress:** split-conformal per-decile bands on calibrated probabilities — not yet publishable. Calibration basis measured: first isotonic fit on the v4.2 logits (Brier 0.086 → 0.081 per-pixel); decile curves + chip-level curves computed; re-threshold mapping (raw 0.50 → cal 0.37) derived; **verification in flight**. Gate: no new government-facing confidence claims until the decile check passes and coverage is re-measured with calibrated probabilities — confidence language frozen.
...
- **Uncertainty band — artifact found, correction in progress (NOT publishable):** the two-sided band was a sigmoid-clipping artifact (w_up = +0.218); the one-sided aggregate (onset coverage 0.822) was **decile-carried** — held up by clipped-to-zero rows, with 1–28% coverage across the 0.5–0.9 deciles. Both are withdrawn from all text. Rebuild in progress: split-conformal per-decile bands on calibrated probabilities. Confidence thresholds/language frozen until verification. Lexicon remains "historical range, not a guarantee"; go-before reads the lower bound, never the midpoint.
